...
3. Find the **Gamma Prompt** section. That is your task.
4. **Build your todo list from it before you write any code.** Break the Gamma Prompt into tasks, and make the last item on that list `Update the Gamma section of coordination.md`. This matters: the skill file you are reading now enters the conversation once and is never re-read. By the time you finish building, it may be far behind you or gone to compaction. Your todo list is the only place the hand-off step reliably survives to the end of a long session.
5. Work through that list, following the API contracts specified in the coordination file.

## Finish

Run these steps when the Gamma Prompt is complete, *before* you tell the user you are done. Reporting completion without them is an incomplete hand-off — Alpha and Delta read the coordination file, not your terminal.
...
3. Run `afplay /System/Library/Sounds/Funk.aiff` to signal completion (macOS only — skip on other platforms).
